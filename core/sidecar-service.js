...
export class SidecarService {
    ...
    async #openAi(settings, request) {
        ...
        const data = await this.#fetchJson(endpoint, { method: 'POST', headers, body: JSON.stringify({ model: settings.model, messages, temperature: request.temperature, top_p: request.sampler.topP, presence_penalty: request.sampler.presencePenalty, max_tokens: request.maxTokens, ...(request.sampler.topK ? { top_k: request.sampler.topK } : {}), ...(request.sampler.minP ? { min_p: request.sampler.minP } : {}), ...(request.sampler.typicalP !== 1 ? { typical_p: request.sampler.typicalP } : {}), ...(request.sampler.repetitionPenalty !== 1 ? { repetition_penalty: request.sampler.repetitionPenalty } : {}), ...(request.sampler.frequencyPenalty ? { frequency_penalty: request.sampler.frequencyPenalty } : {}), ...(request.sampler.seed ? { seed: request.sampler.seed } : {}), ...reasoning }) }, request.timeoutMs, request.signal);
