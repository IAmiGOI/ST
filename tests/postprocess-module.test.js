...
import {
    sanitizePasses, buildPassRequest, cleanPassOutput, runPipeline, applyPipelineResult, postprocessModule,
...
test('sanitizePasses normalizes fields, generates a missing id, bounds lengths, and dedupes by id', () => {
    const passes = sanitizePasses([
        { id: 'a', name: '  Polish  ', prompt: '  Fix grammar.  ', profileId: 'fast', enabled: false },
    ...
    assert.deepEqual(passes[0], { id: 'a', name: 'Polish', prompt: 'Fix grammar.', profileId: 'fast', enabled: false });
...
test('buildPassRequest sends only the pass\'s own instruction and the running text — no chat context, no other pass', () => {
...
test('applyPipelineResult replaces the message and stashes the original text + trace for a future review step', () => {
    const calls = [];
    const context = {
...
test('a reroll (type "regenerate") clears the stale result and reprocesses the new reply', async () => {
    ...
    await listeners.MESSAGE_RECEIVED(0, 'regenerate');
