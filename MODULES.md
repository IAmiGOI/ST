...
Отдельная, более серьёзная защита — для диспетчера `host.onChatChanged()`. На практике встретился реальный баг: листенер одного модуля на входе в чат вызывал `settings()`-геттер своего store, который **безусловно** перезаписывал `chatMetadata` при каждом вызове — даже при простом чтении. Это спровоцировало ST на повторную реакцию, которая снова вызвала `CHAT_CHANGED`, что снова вызвало тот же листенер — цикл, который постепенно съедал память браузера до зависания вкладки/компьютера.

Движок теперь защищён от этого структурно, для всех модулей сразу, а не патчем в одном месте:

- **Реентерабельность.** Если обработка `CHAT_CHANGED` (сихронно, или через что-то, что она вызывает в ST) провоцирует новый `CHAT_CHANGED` **до того**, как первый диспетч завершился, вложенный вызов **отбрасывается** — листенеры не выполняются повторно внутри самих себя.
...
**Практический вывод для авторов модулей**: геттер вида `store.settings()`/`store.get(...)`, который читается на каждый `CHAT_CHANGED` (например, из `inject()`), не должен писать в `chatMetadata`/настройки **безусловно** — только когда значение реально требует нормализации (было не в диапазоне, отсутствовало и т.п.). «Чтение, которое всегда пишет» — именно то, что реентерабельность и призвана ловить, но полагаться на защиту вместо того, чтобы просто не делать так, не стоит: `settings()` в `modules/notebook/store.js` теперь пример правильного паттерна (пишет только при изменении значения).
